...
1. **拿数据（交给脚本）**：运行 `python3 agents/daily-reconcile/scripts/reconcile.py`（直接用完整相对路径，不要用 cd 切换目录，沙箱不放行 cd），它返回一段 JSON：
...
3. **写报告（规范较长，用到才读）**：读 `agents/daily-reconcile/skills/report-format.md` 按它的结构和 P0/P1/P2 分级组织报告；
   某条差异的字段含义或是否属于已知可接受差异拿不准，读 `agents/daily-reconcile/REFERENCE.md` 对照后再定级。

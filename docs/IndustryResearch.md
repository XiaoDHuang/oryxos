...
> 第 28 节范围决议（2026-09-07，用户已批准）：运行时增加可持久查询的定时状态/执行历史、手动执行与启停，由独立 011 承接。它是单实例运行时的操作面，不引入可视化编排、分布式协调或 Agent 治理平台；Agent 定义仍通过统一 Profile 契约。该项目决策不是新增行业调研结论。见[028 范围记录](decisions/028-scheduler-subsystem-preflight.md)。

> 第 27 节实施补充（2026-09-06）：离线脚本模型仅用于可重复的运行时验证，真实模型以外的执行与持久化必须经过实际组件；不能把 mock 测试当作模型能力或企业生产准入的证明。此补充不改变 Agent OS 定位或引入编排能力；实施映射见[第 27 节对账](../specs/010-web-service-admin/lesson27-preflight.md)。

> 007实施补充（2026-08-31）：固定Mem0 1.0.11扫描命中FAISS反序列化告警后，用户批准保留原提炼算法并最小回移官方补丁。受控派生构建不等于升级到新算法，也不代表原版1.0.11已获安全准入；验证边界见[SDK安全回移契约](../specs/007-memory-backends/contracts/sdk-security-backport.md)。
...
### 5.8 Memory 后端的阶段决议（2026-08-30）

OryxOS 继续掌控运行时、核心契约与审计入口；记忆存储后端可以替换，不等于把 ReAct 控制权交给外部 Agent 框架。用户已批准方案 B：006 作为已完成的 Markdown 基线归档，007 在核心阶段增加 SQLite 与可选自托管 Mem0，保留默认单 JAR、本地数据路径。此处是 OryxOS 的范围决议，不是第三方能力已验证或代码已交付的声明。

三种后端不要求检索算法完全相同：Markdown/SQLite 提供关键词匹配，Mem0 可提供经核验的语义检索；共同守住核心记忆完整、归档 scope、写入可见性及审计。自托管 Mem0 不等于自动满足数据不出域：服务、模型、embedding、存储下游及服务内部推理审计都必须检查。默认不开外部记忆连接，不引入遥测；OryxOS 自动对话提炼、图谱和自建向量管线仍在扩展阶段。

范围与模块职责以《需求分析》§5.5、《技术方案》§5/§9/§10 为准；[007 范围记录](decisions/007-memory-backends-scope.md) 承接拆解，不能据此跳过 plan 的协议和安全接线核验。

实施进度补充（2026-09-01）：007默认兼容与SQLite已提交，受控SDK暂存及T031–T032配置/共享协议基础通过；Mem0仍待真实PG、服务及运行门禁，不以本地后端或离线测试证据代替外部组件验收。外部权威状态采用PostgreSQL，Mem0边界明确拒绝其TEXT无法保存的U+0000，不静默改写原文；本地后端不受该限制。

007 plan 核验发现原版 Mem0 服务不足以直接兑现全部契约后，用户已批准增加随 Mem0 部署的受控 Python 适配组件和必需的 HTTP 白名单接线。适配仅管理记忆事务、历史、协议及审计，不接管 Agent 运行时；默认单 JAR 体验和 9 个 Maven 模块不变。外部组件的真实部署及安全性仍须单独验收。

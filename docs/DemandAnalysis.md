...
> 第 28 节范围决议（2026-09-07，用户已批准，011 待实现）：管理员可以查看定时任务状态与执行历史、立即执行及启用/停用；任务状态、历史和已完成会话/记忆/审计跨进程重启保留。规则仍由 Profile 配置提供，Skill 负责业务说明，不在本节引入任务定义创建/删除/修改或动态创建 Agent。多 Profile 的工具、会话和调度执行须互不混淆；失败通知不可被报告为推送成功。管理台新增定时操作页，其余五页继续只读，视觉以 website 为准。天气链路应在取得本次天气后生成通知，再收尾；具体模型/工具对账见[028 决议](decisions/028-scheduler-subsystem-preflight.md)。

> 第 27 节串联决议（2026-09-06）：沿用 010 已批准的分页会话列表，以详情消息总数完成对账；提供显式启用的离线 mock 模型进行无 key 验证，模型以外的记忆、会话、工具和审计保持真实路径。测试使用独立工作区；显式上下文引用缺失必须失败。真实模型验收与 mock 验证分开记录，不能以合成响应代替真实效果。范围及批准记录见[第 27 节对账](../specs/010-web-service-admin/lesson27-preflight.md)。

> 007补充决议（2026-08-31已批准）：固定Mem0算法不变，允许最小官方安全补丁回移及可重复构建。不得以未启用FAISS为默认豁免，必须验证补丁、原算法保留和完整依赖扫描；只处置有源码与运行证明的已修复告警，不放宽记忆/安全需求。详见[SDK安全回移契约](../specs/007-memory-backends/contracts/sdk-security-backport.md)。
...
| 长期记忆 | 用户偏好、项目背景、关键事实；默认 Markdown，007 增加 SQLite / 自托管 Mem0 可选后端 | 006已归档；007默认兼容已提交、SQLite本地验收通过、Mem0待实现 |
...
2026-08-30 用户批准方案 B：保留已归档的 006，另以 007 承接三后端核心范围扩张。2026-08-31已开始实施；以下是完整批准需求，不代表所有后端均已验收。实际阶段及门禁见 [007 范围记录](decisions/007-memory-backends-scope.md)。
...
2026-09-01用户确认远端表示边界：由于外部权威状态使用PostgreSQL TEXT，Mem0的保存正文、查询及模型生成内容不得包含U+0000；遇到时在登记/出站/提交前明确拒绝，不删除、替换或编码改写后冒充原文保全。该限制不改变hash字段之间的NUL分隔算法，也不收紧Markdown/SQLite本地后端。

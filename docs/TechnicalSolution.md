...
- **CLI Channel** 用于本地交互和调试
- **Web Service** 用于业务系统通过 REST API 集成
...
| 能力 | 职责 |
|------|------|
| **Provider** | LLM 调用并向外对接各家大模型 API |
| **Memory** | 会话和长期记忆并读写本地文件 |
| **Tool** | 工具执行并通过 MCP Client 向外对接外部 MCP server |
...
![OryxOS 分层架构](images/architecture.png)
...
| 能力 | 角色 |
|------|------|
| **ReAct 循环（能力二）** | 引擎，负责把「用户消息 → LLM 思考 → Tool 执行 → 结果回填 → 继续」这件事跑起来 |
...
| **Web Service（能力五）** | 这套内部能力的对外出口，把前四个能力包装成 REST API 供业务系统集成；不参与 Agent 内部循环，而是循环的触发入口和结果出口之一（另一个入口是 CLI Channel） |
...
| 模块 | 职责 |
|------|------|
| **ProviderService 模块** | 统一管理所有 LLM Provider，对 ReAct 循环屏蔽不同 LLM 厂商的差异。ReAct 循环调 LLM 时传入 Profile 和 Prompt，`ProviderService` 按 Profile 配置选对应的底层 `ChatModel` 完成调用 |
| **Function Calling 适配模块** | 把 OryxOS 内部的 `OryxTool` 抽象转成 Spring AI 的工具调用格式。Spring AI 已经做好了向各家 LLM 协议的转换（OpenAI tools、Anthropic tools、Gemini function declarations），OryxOS 不需要关心每家协议的差异。注意这里只用 Spring AI 的格式转换，不用它的自动执行（见决策二） |
...
| **PromptBuilder 模块** | 组装每轮 LLM 调用的 Prompt，按四部分顺序拼接：<br>① system prompt（Profile 的 identity prompt 加 Bootstrap 文件加 Skill 文件内容，统一由 `ContextLoader` 提供）<br>② Memory 注入（会话历史加长期记忆，由 `MemoryService` 提供）<br>③ 对话历史（按 `maxHistoryTurns` 截断后的 Session messages）<br>④ 当前 Profile 可用的 Tool 列表（按 Function Calling 格式） |
...
| **会话记忆** | 由 `SessionManager` 实现（见第 8 章），通过 SQLite 持久化，按 Channel 加 user 加 Profile 联合标识管理。`MemoryService` 把它作为三层之一统一对外 |

### 5.2 MEMORY.md 文件设计

- **文件位置：** `.oryxos/memory/MEMORY.md`
- **内容格式：** 简单的 markdown 列表，每条记忆带日期 header。格式不规定严格，Agent 写什么 LLM 自己理解就行，简单但有效。
...
| 文件 | 角色 | 来源 | 生命周期 |
|------|------|------|----------|
| **USER.md** | Bootstrap 文件，用户的「初始设定」 | 由用户手写，OryxOS 只读不写 | 用户维护 |
| **MEMORY.md** | 长期记忆，Agent 的「成长记录」 | 由 Agent 通过 `save_memory` 写入，OryxOS 读写 | Agent 运行时积累 |
...
| `GET` | `/api/v1/memory` | Memory 查询 |
| `GET` | `/api/v1/tools` | Tool 信息 |
...
Profile 的 YAML 包含：`name`、`description`、`identity`（`agent_name`、`prompt`）、`provider`（`name`、`model`、`temperature`）、`tools`、`skills`、`mcp_servers`、`channels`、`bootstrap`、`settings`（`max_iterations`、`max_history_turns`）。

核心阶段支持多个 Profile 并存，多个 Agent 在同一实例上同时可用，这是「OS」在核心阶段的最小体现。
...
|------|------|
| `oryxos chat` | 交互对话 |
| `oryxos serve` | 启动 Web Service |
| `oryxos gateway` | 守护进程同时挂多个 Channel |
...
| `status` | 状态查看 |
| `chat` | 交互对话 |
| `serve` | 启动 Web Service |
| `gateway` | 守护进程 |
| `profile list/create/show/delete` | Profile 管理 |
...
OryxOS 是 Maven 多模块项目。由 **9 个模块**组成：
...
1. 第一次对话用户说项目用 Spring Boot 部署在 K8s 上，DeepSeek 判断值得长期记，调 `save_memory` 追加到 `MEMORY.md`
2. 第二次对话（甚至重启后），`ReActLoop` 组装 Prompt 时 `MemoryService` 把 `MEMORY.md` 注入 system prompt，DeepSeek 看到偏好自然用上，不需要用户重新解释
...
3. 触发后 `ReActLoop` 组装 Prompt 把 `SKILL.md` 注入 system prompt，LLM 自己决定先调 `github-mcp` 拉 PR，`McpToolAdapter` 通过 MCP 协议转发，结果回填，LLM 再调 `slack-mcp` 推送
...
2. `POST /sessions/{id}/messages` 发消息，`AgentController` 调 `AgentService.process` 走完整 `ReActLoop`
3. 响应返回，`GET` 查历史、`DELETE` 归档

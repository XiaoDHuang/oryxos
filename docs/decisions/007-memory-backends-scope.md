...
2026-08-31追加进度：用户已批准最小官方SDK安全回移，T076–T079验证通过，当前28/79项；原提炼算法保留，当前Java完整回归与Python补丁回归由主模型执行。历史Spark安排已被最新指令替代，详见[验收台账](../../specs/007-memory-backends/acceptance.md)。Mem0业务后端仍待实现，007未封板。

2026-09-01追加决议：T031–T032完成后发现PG TEXT不能保全U+0000，用户接受推荐方案。仅Mem0远端content/query及模型生成内容拒绝NUL；hash字段间NUL分隔符和Markdown/SQLite能力不变。随后用户明确要求跳过PG/T045并继续其他内容，T054/T056/T059按顺序例外完成并修绿T053，当前44/79；T045/T051和真实事务/服务仍未实现。

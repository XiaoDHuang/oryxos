...
Provider 让 Agent 会调模型，ReAct 让它会思考，但到现在它还只会"想"和"说"。这节讲的Tool，是让 Agent 真正能动手干事的那双手。四件事照旧：Tool 是什么、动手前该想清楚什么、代码怎么写、怎么用和怎么验。
...
![第20节 图1](images/第20节/图-01.svg)

OryxOS 的 Tool 分两类：

内置 **Tool**：OryxOS 自带的基础工具，核心阶段一共九个—— `read_file` / `write_file` /`list_dir` （文件，3 个）、`shell` （跑命令，1 个）、`http_get` / `http_post` （发请求，2个）、Memory 用的 `save_memory` / `recall_memory` （2 个），再加上 19 节刚讲过的`notify` （推送，1 个）。它们覆盖"读写文件、跑命令、调 API、记事、往外推通知"这条最短链路。（对齐业界主流 Agent 的实践，实现时又补了 `edit_file` / `grep` / `glob` /`ask_user` / `web_search` 五个基础工具，见本节第六部分。）**Plugin Tool**：业务方自己扩展的工具。企业要做运维助手、客服助手，靠的就是它——OryxOS 本身只给基础工具，真正的业务能力是业务方接进来的。
...
第三，所有工具执行前都要过安全校验。 工具能读文件、跑命令、发请求，一旦被乱调就是事故。核心阶段的办法是应用层白名单：文件操作查路径白名单、Shell 查命令白名单、HTTP 查域名白名单，执行前统一过一层校验，校验不过直接拦下——这层校验具体怎么设计是 23、24 节Sandbox 模块的内容，这里先知道执行链路里有这一步。这是核心阶段唯一的 Tool 治理手段，得在设计时就串进执行链路里。
...
关键就两条：注册时把每个 MCP 工具包装成 `OryxTool` 塞进同一个 `ToolRegistry` （ReAct 循环由此对来源无感知）；执行时通过 MCP 协议原样转发、结果包成 `ToolResult` 。失联的server 记 WARN 跳过，不能变成自己的启动故障。
...
看它在干嘛：`@Tool` 注解让 Spring AI 自动把这个方法注册成工具、并根据参数生成 schema；

方法体里第一件事就是过安全校验，域名不在白名单里直接抛异常、请求根本发不出去；过了校验才真正去请求。安全校验写在执行的第一步，这是硬规矩——这里先用一下这个校验，具体`Sandbox` / `SandboxAction` 怎么设计，23、24 节详细展开。
...
![第20节 图3](images/第20节/图-03.svg)

看测试用例。 Plugin Tool 写完得能验。一个 Java Plugin Tool 的测试其实很朴素——给输入、调execute、断言结果，顺便验一下白名单会拦（这两个用例会收编进下面第四部分的 harness）：

```java
@Test
void http_get_应能取回响应() {
    var result = httpTool.execute(json("
...
代码：`OryxTool` 接口（含 `getInputSchema` ）、`ToolResult` 、`ToolRegistry` 、`AnnotatedToolAdapter` 、`FileTools` （3 个）、`ShellTools` 、`HttpTools` （2 个）、`McpClientService` 、`McpToolAdapter` ；`NotifyTools` （19 节）在此完成注册；

`Sandbox` 前向接口五件（含 `PermissiveSandbox` 临时装配，实现本体 24 节）
...
说明：白名单校验先以接口调用形式接入，`Sandbox` 本体 23/24 节交付扩展交付（对齐业界主流，见第六部分）：`edit_file` / `grep` / `glob` （扩进`FileTools` ）、`ask_user` （`InteractionTools` + `UserInteraction` 抽象）、`web_search`（`WebSearchTools` + `SearchProvider` 抽象）
...
```java
@ParameterizedTest
@MethodSource("allRegisteredTools")   // 遍历 ToolRegistry，新工具自动纳入契约检
查void 每个工具的契约三件套都不能缺(OryxTool tool) {
...
按 Profile 过滤的测试有个"不多不少"的讲究：断言子集恰好等于声明列表，多一个（没过滤干净）和少一个（过滤过头）都是错。
...
方式一（零代码）：在 .oryxos/skills/ 写一份 SKILL.md，Profile 里 skills 引用它、
                  mcp_servers 引用要复用的 MCP server。方式二（轻代码）：在 .oryxos/mcp_servers.yaml 声明你自己的 MCP server，OryxOS 启动时连上。方式三（重代码）：写一个带 @Tool 注解的 Spring Bean，放进工程，启动自动注册。
```
...
规划的九个内置工具覆盖了"读写文件、跑命令、调 API、记事、往外推"的最短链路。但对着Claude Code、Cursor、OpenAI Assistants 这些主流 Agent 产品的公约数一比，会发现少了几样日常任务里高频、缺了就明显笨手笨脚的工具。所以实现时又补了五个——它们不是新概念，全都复用本节已立好的 `OryxTool` 抽象、`@Tool` 注解管道和 `Sandbox` 检查位，正好也是对前面那套设计的一次实战检验：加工具确实只是"多写一个方法 / 多挂一个实现类"。
...
把文件里一段唯一出现的只有整文件覆盖写（`write_file` ），改一行配置要把整个文件重写一遍——既费 token 又容易把别旧文本换成新文本的内容冲掉

```
grep
```

在路径下按正则搜内容，Agent 想"查一下哪个配置引用了 X"只能一层层返回 `文``件``:``行``号``:``内``容``list_dir` + `read_file` 硬翻按通配（如 `**/*.yaml` ）同上，"找出所有 yaml"没有顺手的办法

```
glob
```

找文件路径处理中途向用户提问 / 请危险操作没法找人拍板，human-in-the-loop 没有
...
用搜索引擎检索网页日报 / 调研类 Agent 少了"上网查"的第一入口，只能靠业务方各自配 MCP判断标准还是那句话：这几样是**"**基础工具**"**而非**"**业务能力**"**——任何一个稍复杂的任务都会用到，所以值得进内置清单，而不是丢给 Plugin Tool 每家各写一遍。
...
值得回头看一眼加这五个工具动了多少地方，因为这正好印证了前面那套抽象的价值：

三个文件工具：`FileTools` 里加三个 `@Tool` 方法，没动别的。

`ask_user` / `web_search` ：各加一个工具类 + 一个前向接口 + 一档实现，跟 19 节加notify、24 节加 Sandbox 是一模一样的动作。

...
> **2026-08-30 决议更新**：下文保留原评审背景与 006 文件基线的取舍；“核心阶段仅文件、Mem0/语义检索只在扩展”的旧阶段判断已由用户批准的方案 B 调整。006 在 `3d60ee0` 归档，007 在核心阶段承接 Markdown 默认 + SQLite / 自托管 Mem0 可选后端，不改变引擎端口和 9 模块。当前规则见文末新增决议及《技术方案》§5/§9；不追溯改写 006 验收结果。
...
### 追加决议：006 稳定基线，007 三后端续篇

- 保留 `MemoryService` / `MemoryScope` 核心端口与既有签名，memory 内增加 Store 适配；Session 不迁入新的记忆模型。
- Markdown 默认、SQLite 与自托管 Mem0 显式可选。核心全量、归档 scope、保存可见性是共同契约；关键词/语义检索和归档窗口是后端差异，不强行宣称等价。
- Mem0 是可选记忆层，不是替代运行时。批准集成范围不等于已验证其协议和部署；服务、模型、embedding、存储下游及审计须在 plan 核验，数据不出域与出站校验不可省。
- OryxOS 自动对话提炼、图谱、压缩、自建向量层仍属扩展；Mem0 显式保存时的后端推理需单独核验，不能改写核心原文或假称内部调用已落 OryxOS 审计表。
- 第 22 节拆成已完成的 006 与待实现的 007。故事、后端矩阵和未决门禁见 [007 范围记录](../decisions/007-memory-backends-scope.md)，不把原来条件性的业界判断当成实现许可。
